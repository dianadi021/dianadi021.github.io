...
function popup_result_TOPSIS() {
  ...
  */
  ...
  var innerHTMLSimpleResultLast = `</ul>
                        ...
                        <a id="downloadJSON" onclick="showTable();" type="button" data-toggle="modal" data-target="#topsisFullResult" class="btn btn-info btn-lg btn-block">Show Table <span class="oi oi-eye "></span></button></a>
                      </div>
                    </div>
                  </div>`;
  ...
  for (let iChoicesLength = 1; iChoicesLength <= choicesArrayLength; iChoicesLength++) {
    ...
    for (let index = 1; index <= choicesArrayLength; index++) {
      ...
      if (allChoicesValue[iChoicesLength - 1] == objIndicator.nilaiPreferensiAlternatif[index - 1].NilaiPreferensi) {
        innerHTMLSimpleResultMiddle +=
          `<li class="list-group-item "><strong>Rank ` +
          iChoicesLength +
          ` </strong>` +
          objIndicator.nilaiPreferensiAlternatif[index - 1].Choice +
          `</strong> dengan nilai <strong>` +
...
<td><strong>Cost/Benefit</strong></td>
  ...
  for (let iIndicatorLength = 1; iIndicatorLength <= indicatorArrayLength; iIndicatorLength++) {
    ...
    if (objIndicator.maxMinPlusNegatifA[iIndicatorLength - 1].MatrixOf == tempNameIndicator) {
      tempAPlusNegatifShow +=
        `<td><strong>` +
        objIndicator.maxMinPlusNegatifA[iIndicatorLength - 1].MatrixOf +
        `</strong></td><td>` +
        objIndicator.maxMinPlusNegatifA[iIndicatorLength - 1].plusA.toFixed(3) +
        `</td><td>` +
        objIndicator.maxMinPlusNegatifA[iIndicatorLength - 1].negatifA.toFixed(3) +
        `</td></tr>`;
    }
  }
  ...
  for (let iChoicesLength = 1; iChoicesLength <= choicesArrayLength; iChoicesLength++) {
    ...
    if (objIndicator.nilaiPlusNegatifD[iChoicesLength - 1].Choice == tempNameChoices) {
      tempDPlusNegatifShow +=
        `<td><strong>` +
        objIndicator.nilaiPlusNegatifD[iChoicesLength - 1].Choice +
        `</strong></td><td>` +
        objIndicator.nilaiPlusNegatifD[iChoicesLength - 1].plusD.toFixed(3) +
        `</td><td>` +
        objIndicator.nilaiPlusNegatifD[iChoicesLength - 1].negatifD.toFixed(3) +
        `</td></tr>`;
    }
  }
  ...
  var tempBtnDownloadXLS = `<div id="margin-top-byme" class="row">
  <div class="col-sm-12">
    <center><a id="downloadJSON" onclick="ExportToExcel('xlsx');" type="button" data-toggle="modal" data-target="#topsisFullResult" class="btn btn-info btn-lg btn-block">Download File Excel <span class="oi oi-file "></span></button></a></center>
  </div>
</div>`;

  document.getElementById("tbl_exporttable_to_xls").innerHTML =
    tempChoicesShow + tempIndicatorShow + tempAllMatrixShow + tempNilaiAkarKuadrat + tempNilaiTernormalisasi + tempNilaiTernormalisasiTerbobot + tempAPlusNegatifShow + tempDPlusNegatifShow + tempNilaiPilihanAlternatifShow + tempBtnDownloadXLS;
}
